...
use Exception;
use HelgeSverre\ReceiptScanner\Services\Textract\Data\AnalyzeExpenseResponse;
...
class TextractService
{
    ...
    public function analyzeExpenseFromBytes(string $content): ?string
    {
        ...
        return AnalyzeExpenseResponse::fromAwsResult($result)?->getText();
    }

    public function s3ObjectToExpenseText(S3Object $s3Object, int $timeoutInSeconds = 60, int $pollingIntervalInSeconds = 1): ?string
    {
        $result = $this->textractClient->startExpenseAnalysis([
            'ClientRequestToken' => $s3Object->getClientRequestToken(),
            'DocumentLocation' => [
                'S3Object' => array_filter([
                    'Bucket' => $s3Object->bucket,
        ...
        while (true) {
            ...
            if ($elapsedTime >= $timeoutInSeconds) {
                throw new Exception('Textract expense analysis job timed out.');
            }
            ...
            $response = $this->textractClient->getExpenseAnalysis(['JobId' => $jobId]);
            $status = Arr::get($response, 'JobStatus');

            switch ($status) {
                case self::STATUS_SUCCEEDED:
                    return AnalyzeExpenseResponse::fromAwsResult($response)?->getText();
                ...
                case self::STATUS_PARTIAL_SUCCESS:
                case self::STATUS_FAILED:
                    throw new Exception("Textract expense analysis job failed with status '$status'.");
                default:
                    throw new Exception("Unhandled Textract expense analysis job status: '$status'.");
            }
        }
    }
}
